        ...
        <div className="idsk-search-results__filter idsk-search-results-filter__filter-panel">
            <div className="idsk-search-results__link-panel idsk-search-results--expand idsk-search-results__content-type-filter">
                ...
                <div className="idsk-search-results__list">
                    <input className="govuk-input idsk-search-results__search__input" type="text" value={query} onChange={e => setQuery(e.target.value)} title={props.searchElementTitle}  aria-label={props.searchElementTitle} />
                    <div className="idsk-option-select-filter ">
                        <div className="govuk-form-group">
                            <div className="govuk-checkboxes govuk-checkboxes--small">
                                {items.map((item, index) => <Checkbox key={index} label={props.getLabel(item)} checked={selectedValues.includes(props.getValue(item))} onCheckedChange={c => changeItemSelection(item, c)} />)}
                            </div>
                        </div>
                    </div>
                </div>
